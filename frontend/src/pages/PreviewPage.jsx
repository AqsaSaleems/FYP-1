...
export default function PreviewPage() {
  ...
  const drawDesignOnCanvas = (canvas, design, side) => {
    ...
    const drawOverlays = () => {
      ...
      // Draw sticker: supports emoji, object with emoji, or image url
      if (design.sticker) {
        const stickerSize = (design.stickerSize || 80) * scale;
        const stickerX = design.stickerPos?.x ? (design.stickerPos.x / 100) * width : width * 0.7;
        const stickerY = design.stickerPos?.y ? (design.stickerPos.y / 100) * height : height * 0.65;

        if (typeof design.sticker === 'string') {
          // string could be emoji or url/data
          if (design.sticker.startsWith('data:') || design.sticker.startsWith('http') || design.sticker.startsWith('blob:')) {
            drawImageFromSource(design.sticker, stickerX - stickerSize / 2, stickerY - stickerSize / 2, stickerSize, stickerSize);
          } else {
            ctx.font = `${stickerSize}px Arial`;
            ctx.textAlign = "center";
            ctx.textBaseline = "middle";
            ctx.fillText(design.sticker, stickerX, stickerY);
          }
        } else if (typeof design.sticker === 'object') {
          if (design.sticker.emoji) {
            ctx.font = `${stickerSize}px Arial`;
            ctx.textAlign = "center";
            ctx.textBaseline = "middle";
            ctx.fillText(design.sticker.emoji, stickerX, stickerY);
          } else if (design.sticker.url) {
            drawImageFromSource(design.sticker.url, stickerX - stickerSize / 2, stickerY - stickerSize / 2, stickerSize, stickerSize);
          }
        }
      }
    };
    ...
    <div style={styles.container}>
      ...
      <div style={styles.previewSection}>
        ...
        <div style={styles.detailsContainer}>
          ...
          {(normalizedFront.text || normalizedFront.logo || normalizedFront.sticker || 
            normalizedBack.text || normalizedBack.logo || normalizedBack.sticker) && (
            <div style={styles.customizationBox}>
              ...
              {(normalizedFront.text || normalizedFront.logo || normalizedFront.sticker) && (
                <div>
                  ...
                  {normalizedFront.sticker && <p style={styles.detailText}>Sticker: {typeof normalizedFront.sticker === 'object' ? (normalizedFront.sticker.name || JSON.stringify(normalizedFront.sticker)) : normalizedFront.sticker}</p>}
              ...
              {(normalizedBack.text || normalizedBack.logo || normalizedBack.sticker) && (
                <div>
                  ...
                  {normalizedBack.sticker && <p style={styles.detailText}>Sticker: {typeof normalizedBack.sticker === 'object' ? (normalizedBack.sticker.name || JSON.stringify(normalizedBack.sticker)) : normalizedBack.sticker}</p>}
